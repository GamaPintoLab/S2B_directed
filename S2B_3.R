S2B_3=function(seed_graph,index1,index2,nrep,nrep2){
  meandist=igraph::mean_distance(seed_graph, directed = TRUE)
  bt=subS2B_version3(seed_graph,index1,index2,meandist) # specify subS2B version to be used to compute S2B scores
  ...
  if (nrep>0){
    ...
    for (i in 1:nrep){
      ...
      rbt=subS2B_version3(seed_graph,rindex1,rindex2,meandist) # specify subS2B version, S2B scores after shuffle of seed identity
  ...
  if (nrep2>0){
    ...
    for (i in 1:nrep2){
      ...
      rbt=subS2B_version3(rg,index1,index2,meandist) # specify subS2B version, S2B scores after shuffle of edges
